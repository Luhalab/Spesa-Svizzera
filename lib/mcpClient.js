...
async function getClient() {
  ...
  clientPromise = (async () => {
    // Cartella HOME unica per questo tentativo di connessione: se il
    // pacchetto salva su disco uno stato di blocco/rate-limit per una
    // catena, non deve poterlo rileggere da un tentativo precedente —
    // /tmp persiste tra le richieste sullo stesso server Vercel.
    const uniqueHome = `/tmp/sgm-${Date.now()}-${Math.random().toString(36).slice(2)}`;
    ...
    const transport = new StdioClientTransport({
      ...
      env: {
        ...
        // Coop usa la protezione anti-bot DataDome, che blocca le richieste
        // che non sembrano provenire da un vero iPhone. Comportamento
        // documentato dal pacchetto stesso: senza questo, Coop fallisce in
        // silenzio (nessun errore, solo risultati vuoti).
...
// Scarta la connessione attuale, forzando la prossima chiamata ad aprirne
// una nuova da zero. Utile quando la connessione esistente sembra "bloccata"
// per una catena specifica (es. Coop): un retry sulla STESSA connessione
// non cambia nulla, serve una connessione (e quindi una sessione) diversa.
